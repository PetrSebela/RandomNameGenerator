...
public class RandomNames
{
    ...
    private HashSet<string> generatedNames = new();
    ...
    public string _broken_GetUniqueRandomName(string separator ="_")
    {
        if (generated >= possible)
            throw new Exception("You somehow generated all possible names");

        int limiter = 10;
        string name = GetRandomName(separator);
        while (generatedNames.Contains(name) && limiter >= 0)
        {   
            name = GetRandomName(separator);
            limiter--;
        }
