# Originaly https://github.com/docker/docker-ce/blob/master/components/engine/pkg/namesgenerator/names-generator.go
# I do not take any credit for this work, i just ported it to python and slightly modified it
...
def get_random_name(separator:str="_") -> str:
    ...
    return f"{adjective}{separator}{surname}"


existing_names = []
generated = 0
possible = len(adjectives) * len(surnames)


def get_unique_random_name(separator:str="_") -> str:
    # same as get_random_name but unique
    # i know the chanses of generating two same names are close to zero, but just in case close to zero is not good enough
    global generated

    if generated >= possible:
        raise Exception("You somehow generated all possible names")

    name = get_random_name(separator)

    while name in existing_names or name == f"boring{separator}einstein": # einstein is not boring (if you dont get this, read the original code ;) ) 
        name = get_random_name(separator)
